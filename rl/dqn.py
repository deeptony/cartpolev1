...
class QPolicy():
    def __init__(self, q, env_act_space, eps=0.1, gamma=0.99, pt_dev='cpu'):
        self.eps = eps
        self.gamma = gamma
    ...
    def __call__(self, obs): # epsilon-greedy policy to encourage exploration
        if np.random.rand() <= self.eps: 
            return ttl(self.env_act_space.sample()).to(self.pt_dev), torch.zeros(self.env_act_space.n).to(self.pt_dev) # follow random policy 
        else:
            with torch.no_grad():
