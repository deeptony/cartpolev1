...
class Agent(): 
    def __init__(self, env, policy, memory, pt_dev):
    ...
    def __iter__(self): # with these two functions we can iterate through the episode
        self.c_obs = ttf(self.env.reset().copy()).to(self.pt_dev) # reset function returns the first observation
    ...
    def __next__(self): # with these two functions we can iterate through the episode
        if self.is_end_state:
            ...
            obs_i, rw_i, end_state, _ = self.env.step(act_i.cpu().numpy() if type(self.env.action_space) is Box else act_i.item())
